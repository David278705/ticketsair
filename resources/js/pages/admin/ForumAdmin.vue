<template>
    <div class="h-screen flex flex-col bg-slate-50 overflow-hidden">
        ...
        <!-- Header -->
        <div
            class="bg-white border-b border-slate-200 px-4 md:px-6 py-3 md:py-4 flex-shrink-0"
        >
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p
                        class="text-sm md:text-base text-slate-600 hidden md:block"
                    >
                        Administra conversaciones con usuarios
                    </p>
                </div>
                <div class="flex items-center gap-2 md:gap-4">
                    <div
                        class="bg-red-100 text-red-800 px-2 md:px-3 py-1 rounded-full text-xs md:text-sm font-medium"
                    >
                        {{ totalUnread }} sin leer
                    </div>
                    <button
                        @click="refreshAll"
                        class="bg-blue-600 text-white px-3 md:px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2"
                    >
                        <RefreshCw
            ...
            <nav class="flex px-4 md:px-6">
                <button
                    ...
                    :class="[
                        ...
                        'py-3 px-3 md:px-6 border-b-2 font-medium text-sm transition-colors flex items-center gap-2',
                    ]"
                >
                    <MessageCircle class="w-4 h-4 md:w-5 md:h-5" />
                    <span class="hidden sm:inline">Foro Público</span>
                </button>
                <button
                ...
                >
                    <Lock class="w-4 h-4 md:w-5 md:h-5" />
                    <span class="hidden sm:inline">Soporte Privado</span>
                    <span
                        v-if="privateUnread > 0"
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            >
                <div class="p-3 md:p-4 border-b border-slate-200 flex-shrink-0">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2
                            ...
                            <p class="text-xs md:text-sm text-slate-600 mt-1">
                                Responde consultas públicas de los usuarios
                            </p>
                        </div>
                        <button
            ...
            <div v-else class="flex flex-1 overflow-hidden">
                ...
                <div
                    class="w-full md:w-1/3 bg-white border-r border-slate-200 flex flex-col overflow-hidden"
                >
                    <div
                        class="p-3 md:p-4 border-b border-slate-200 flex-shrink-0"
                    >
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Buscar conversaciones..."
                            class="w-full px-3 py-2 text-sm md:text-base border border-slate-300 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        />
                    </div>
                    ...
                    <div
                        v-else
                        class="flex-1 overflow-y-auto divide-y divide-slate-200"
                    >
                        <div
                            v-for="thread in filteredThreads"
                            :key="thread.id"
                            @click="selectThread(thread)"
                            class="p-3 md:p-4 hover:bg-slate-50 cursor-pointer transition-colors"
                            :class="{
                        ...
                        >
                            <div class="flex items-start gap-2 md:gap-3">
                                <div class="flex-shrink-0">
                                    <div
                                        class="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-300 flex items-center justify-center text-white font-semibold text-sm"
                                    >
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    >
                                        <h3
                                            class="font-medium text-sm md:text-base text-slate-900 truncate"
                                        >
                                        ...
                                        <span
                                            ...
                                            class="flex-shrink-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold text-white bg-red-500 rounded-full"
                                        >
                                            {{ thread.unread_count }}
                                        </span>
                                    </div>
                                    <p
                                        class="text-xs md:text-sm text-slate-600 truncate"
                                    >
                                        {{ thread.title }}
                                    </p>
                                    <p
                ...
                <div
                    class="hidden md:flex flex-1 flex-col overflow-hidden bg-white"
                    v-if="selectedThread"
                >
                    <div
                    ...
                    >
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-slate-300 flex items-center justify-center text-white font-semibold text-sm md:text-lg"
                                >
                                ...
                                <div>
                                    <h2
                                        class="text-lg md:text-xl font-semibold text-slate-900"
                                    >
                                        {{ selectedThread.user.first_name }}
                                        {{ selectedThread.user.last_name }}
                                    </h2>
                                    <p
                                        class="text-xs md:text-sm text-slate-600"
                                    >
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    v-else
                                    class="px-3 py-1 text-sm bg-slate-100 text-slate-600 rounded-lg"
                                >
                                    Cerrado
                                </span>
                            </div>
                    ...
                    >
                        ...
                        >
                            <textarea
                                ...
                                class="flex-1 px-3 py-2 text-sm md:text-base border border-slate-300 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none disabled:bg-slate-100 disabled:cursor-not-allowed"
                            ></textarea>
                            <button
                                type="submit"
                                :disabled="
                                    !privateMessageText.trim() ||
                                    sendingPrivate ||
                ...
                <div
                    v-else
                    class="hidden md:flex flex-1 items-center justify-center bg-slate-50"
                >
                    <div class="text-center p-4">
                        <MessageCircle
                        ...
                        />
                        <p
                            class="text-base md:text-lg font-medium text-slate-900"
                        >
...
const privateUnread = computed(() => {
    return privateThreads.value.reduce(
...
const filteredThreads = computed(() => {
    ...
    return privateThreads.value.filter((thread) => {
        const userName =
            `${thread.user.first_name} ${thread.user.last_name}`.toLowerCase();
        ...
        if (data.status === "success") {
            if (isAutoRefresh && privateThreads.value.length > 0) {
                ...
                privateThreads.value.forEach((thread, index) => {
                    const updatedThread = newThreads.find(
                        (t) => t.id === thread.id
                    );
                    if (updatedThread) {
        ...
        if (isAutoRefresh && selectedThreadMessages.value.length > 0) {
            const lastMessageId =
                selectedThreadMessages.value[
                    selectedThreadMessages.value.length - 1
                ].id;
            params.since_message_id = lastMessageId;
        }

        const { data } = await api.get(`/forum/threads/${thread.id}`, {
            params,
        });
...
const sendPrivateMessage = async () => {
    ...
    try {
        const { data } = await api.post(
            `/forum/threads/${selectedThread.value.id}/messages`,
            {
        ...
        if (data.status === "success") {
            ...
            const threadInList = privateThreads.value.find(
...
const closeThread = async (thread) => {
    ...
    try {
        const { data } = await api.patch(
            `/admin/forum/threads/${thread.id}/close`
        );
        if (data.status === "success") {
